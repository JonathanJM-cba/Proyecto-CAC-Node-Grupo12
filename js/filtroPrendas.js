...
const btnBuscar = document.querySelector(".btn.btn-primary");

function mostrarPrendas(filtradas) {
  listPrendas.innerHTML = ""; // Para limpiar la lista de prendas

  if (filtradas.length === 0) {
    const mensaje = document.createElement("p");
    mensaje.textContent = "No hay prendas disponibles en esta categoría.";
    listPrendas.appendChild(mensaje);
    return;
  }

  //Se carga cada etiqueta necesarias para mostrar de cada prenda
  filtradas.forEach((prenda) => {
...
// Para mostrar todas las prendas por defecto al cargar la pagina
mostrarPrendas(prendas);

btnBuscar.addEventListener("click", () => {
  const categoriaSeleccionada = selectPrenda.value;
  if (!categoriaSeleccionada) {
    /*
    const alerta = document.createElement("div");
    alerta.classList.add("alert", "alert-danger", "mt-3");
    alerta.role = "alert";
    alerta.textContent = "Debe seleccionar una categoría.";
    listPrendas.innerHTML = "";
    listPrendas.appendChild(alerta);
    */
    mostrarPrendas(prendas);
  } else {
    const prendasFiltradas = prendas.filter(
      (prenda) => prenda.categoria === categoriaSeleccionada
    );
